...
import 'package:flutter_rock_ui/aliyun_oss/better_aliyun_oss_signer.dart';
import 'package:intl/date_symbol_data_local.dart';
import 'package:intl/intl.dart';
import 'package:tuple/tuple.dart';
import 'package:xml/xml.dart';

import 'better_aliyun_oss_credentials.dart';
import 'better_aliyun_oss_http.dart';
import 'better_aliyun_oss_part.dart';
...
class BetterAliyunOssClient {
  ...
  // 每个请求都会有一个监听
  final Map<int, StreamSubscription> _requestSubscriptionMap = {};

  // 阿里上传过程中的事件分发
  final StreamController<BetterAliyunOssClientEvent> _controller = StreamController.broadcast();
  ...
  BetterAliyunOssClientRequest putObject({
    ...
    handle() async* {
      ...
      if (path == null && buffer == null) {
        ...
        _controller.sink
            .add(BetterAliyunOssClientEvent(requestTaskId, BetterAliyunOssClientEventEnum.failure, data: exception));
        return;
      }
      ...
      try {
        ...
        if (path != null) {
          ...
          if (exists == false) {
            ...
            _controller.sink.add(
                BetterAliyunOssClientEvent(requestTaskId, BetterAliyunOssClientEventEnum.failure, data: exception));
            return;
          }
        ...
        Map<String, dynamic> headers = {
          ...
          "x-oss-traffic-limit": 8 * 1024 * 1000,
        };
        ...
        await BetterAliyunOssDioUtils.getInstance(enableLog).put<void>(
          ...
          onSendProgress: (int count, int total) {
            // 上传进度
            _controller.sink.add(BetterAliyunOssClientEvent(requestTaskId, BetterAliyunOssClientEventEnum.progress,
        ...
        // 上传成功
        _controller.sink
            .add(BetterAliyunOssClientEvent(requestTaskId, BetterAliyunOssClientEventEnum.success, data: finallyUrl));
      } catch (e) {
        // 上传失败
    ...
    _requestSubscriptionMap[requestTaskId] =
        handle().listen((event) {}, onDone: () => _requestSubscriptionMap.remove(requestTaskId));
    return BetterAliyunOssClientRequest(requestTaskId: requestTaskId, cancelToken: cancelToken);
  }
    ...
    handle() async* {
      ...
      try {
        ...
        if (eTag != null && eTag.isNotEmpty) {
          // 上传成功
          _controller.sink.add(BetterAliyunOssClientEvent(requestTaskId, BetterAliyunOssClientEventEnum.success,
              data: eTag.first.toString()));
        } else {
          // 上传失败
          final exception = BetterAliyunOssClientException(message: "上传失败");
          _controller.sink
  ...
  ///
  /// return: 返回最终的URL
  Future<Tuple2<String?, BetterAliyunOssClientException?>> completeMultipartUpload(
      {required List<BetterAliyunOssPart> partList}) async {
    // 获取Oss签名
    final verifyCredentialsResult = await verifyCredentials();
    if (!verifyCredentialsResult) {
      final exception = BetterAliyunOssClientException(message: "阿里云Oss签名失败");
    ...
    try {
      ...
      final String requestUrl =
          'https://${partList[0].bucket}.${partList[0].endpoint}/${partList[0].objectPath}?uploadId=${partList[0].uploadId}';
