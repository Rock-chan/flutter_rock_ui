...
TransitionBuilder FxTransitionBuilder({TransitionBuilder? builder}) {
  return (context, child) {
    ...
    var deviceShortWidth = ui.window.physicalSize.width <= ui.window.physicalSize.height
    ...
    Info.init(actualPixelRatio: actualPixelRatio, uiBlueprints: _uiBlueprints, enableLog: _enableLog)
        .onScreenMetricsChange(old);
    if (_enableLog) print("$tag Info=${Info.instance}");
...
class FxWidgetsFlutterBinding extends WidgetsFlutterBinding {
  ...
  late final Resampler _resampler = Resampler(
  ...
  double get adapterDevicePixelRatio {
    ...
    var deviceShortWidth = ui.window.physicalSize.width <= ui.window.physicalSize.height
        ? ui.window.physicalSize.width
        : ui.window.physicalSize.height;
    adapterDevicePixelRatio = deviceShortWidth / _uiBlueprints.width;
    return adapterDevicePixelRatio;
  }
}

class _RoorRenderObjectWidget extends SingleChildRenderObjectWidget {
  const _RoorRenderObjectWidget(Widget rootChild) : super(child: rootChild);
  ...
  RenderObject createRenderObject(BuildContext context) {
    _assertOnFuture(() => !Info.initialed,
        errorMsg: "\nError:'FxTransitionBuilder' is not configured"
            ...
            "\n  ......"
            "\n);");
    return RenderPadding(padding: const EdgeInsets.all(0));
  }
...
class Resampler {
  Resampler(this._handlePointerEvent, this._handleSampleTimeChanged, this._samplingInterval);
