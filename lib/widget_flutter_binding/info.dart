...
class Info {
  ...
  factory Info.init(
      {required double actualPixelRatio, required BlueprintsRectangle uiBlueprints, bool enableLog = false}) {
  ...
  void onScreenMetricsChange(MediaQueryData queryData) {
    ...
    if (enableLog) log("原MediaQueryData=$queryData \n vertical=$vertical");
    if (isVertical) {
  ...
  void _printInfo() {
    if (!enableLog) return;
    log("$tag △L=$deltaLength 状态栏△L=${_deltaPadding.vertical} _bodyMaxLength=$_bodyMaxLength");
    log("$tag 设计稿标准尺寸 = $uiBlueprints h/w tgθ= ${1 / uiBlueprints.aspectRatio}");
    log("$tag 设备的屏幕尺寸 =${window.physicalSize}  h/w tgθ=${1 / window.physicalSize.aspectRatio}");

    if (deltaLength > 0) {
      log("$tag 尺码过小 △L=$deltaLength 设置需要留白的区域");
    } else if (deltaLength < 0) {
      log("$tag 尺码过大 △L=$deltaLength 请启用纵向滑动式适配");
    } else {
      log("$tag 尺码完全符合");
    }
    log("$tag 原本屏幕尺寸比率=$devicePixelRatio");
    log("$tag 转换后的屏幕尺寸比率=$actualPixelRatio");
  }
